...
$().ready(function () {
    ...
    $('#divTabEncuestas').jqxTabs('theme', 'oasis');
    $('#divTabEncuestas').jqxTabs('enableAt', 0);
    $('#divTabEncuestas').jqxTabs('disableAt', 1);
    $('#divTabEncuestas').jqxTabs('disableAt', 2);

    $('#tabFichaPersonalEncuestas').jqxTabs({
        theme: 'oasis',
        width: '100%',
        position: 'top'
    });
    $('#tabFichaPersonalEncuestas').jqxTabs({selectedItem: 0});
    var dataRecordMain = getOneRelaboralById($("#hdnIdRelaboralUsuario").val());
    cargarDatosEncuestas(dataRecordMain);

    $("#liEncuestas").on("click",function () {
        $('#divTabEncuestas').jqxTabs('enableAt', 0);
        $('#divTabEncuestas').jqxTabs('disableAt', 1);

        $('#divTabEncuestas').jqxTabs({selectedItem: 0});
    });
    /**
     * Control del evento de solicitud de guardar el registro de la Idea de Negocio.
     */
    $("#btnGuardarEncuestaNew").on("click",function () {
        if(confirm("¿Esta segur@ de las respuestas que registró? Considere que una vez registrada la encuesta ya no podrá realizar modificaciones.")){
            var sufijo = "New";
            var contadorPreguntas=0;
            var contadorPreguntasContestadas=0;
            $.each($(".preg"), function() {
                var valor = (this.id).split("_");
                var tipoPregunta = parseInt(valor[1]);
                var idPregunta = parseInt(valor[2]);
                var idRelaboral = $("#hdnIdRelaboral"+sufijo).val();
                var idEncuesta = $("#hdnIdEncuesta"+sufijo).val();
                contadorPreguntas++;
                switch (tipoPregunta){
                    case 1:
                        break;
                    case 2:
                        break;
                    case 3:
                        $.each($(".resp_"+idPregunta), function() {

                            if($(this).prop("checked")===true){
                                contadorPreguntasContestadas++;
                                var idOpcionRespuesta = 0;
                                var val = (this.id).split("_");
                                var idOpcionRespuesta = parseInt(val[1].replace("or",""));
                                var ctrlAgregacion = parseInt(val[2]);
                                var agregacion = "";
                                var observacion = $("#txtComentarioEncuesta"+sufijo).val();
                                if(ctrlAgregacion==1){
                                    var idAgregacion = "agr_p"+idPregunta+"_or"+idOpcionRespuesta;
                                    agregacion = $("#"+idAgregacion).val();
                                }
                                var ok = guardarRespuestaOpcionalPorRadio(idRelaboral,idEncuesta,idPregunta,idOpcionRespuesta,agregacion,observacion);
                            }
                        });
                        break;
                }
            });
            if(contadorPreguntasContestadas==0){
                var msje = "Al menos debe contestar una pregunta para que su formulario se registre.";
                $("#divMsjePorError").html("");
                $("#divMsjePorError").append(msje);
                $("#divMsjeNotificacionError").jqxNotification("open");
            }else{
                if(contadorPreguntasContestadas==contadorPreguntas){
                    ...
                    $('#divTabEncuestas').jqxTabs({selectedItem: 0});
                    if(contadorPreguntasContestadas==contadorPreguntas){
                        var msje = "Registro Satisfactorio de las respuesta de su encuesta.";
                        $("#divMsjePorSuccess").html("");
                        $("#divMsjePorSuccess").append(msje);
                        $("#divMsjeNotificacionSuccess").jqxNotification("open");
                    }else{
                        var msje = "Registro satisfactorio de las preguntas contestadas en su encuesta, sin embargo, quedan pendientes de ser respondidas algunas preguntas.";
                        $("#divMsjePorWarning").html("");
                        $("#divMsjePorWarning").append(msje);
                        $("#divMsjeNotificacionWarning").jqxNotification("open");
                    }
                }else{
                    var msje = "Debe responder todas las preguntas necesariamente. En caso contrario sus dem&aacute;s respuestas no serán consideradas.";
                    $("#divMsjePorWarning").html("");
                    $("#divMsjePorWarning").append(msje);
                    $("#divMsjeNotificacionWarning").jqxNotification("open");
                }
    ...
    $("#btnCancelarEncuestaNew,#btnVolverAlListadoPrincipalDesdeNew,#btnVolverAlListadoPrincipalDesdeView").click(function () {
        $('#divTabEncuestas').jqxTabs('enableAt', 0);
        $('#divTabEncuestas').jqxTabs('disableAt', 1);
        $('#divTabEncuestas').jqxTabs('disableAt', 2);
        $('#divTabEncuestas').jqxTabs({selectedItem: 0});
    ...
    $(".iObservacion").on("click",function(){
        ...
        var cuerpo = "Registre ac&aacute; sus comentarios.";
        var alerta = "";
 ...
 * @param ci
 */
function cargarDatosEncuestas(dataRecordMain){
    ...
    cargarPersonasContactosEncuestas(2,dataRecordMain.id_persona);
    $("#hdnIdRelaboralVistaTurnAndExcept").val(dataRecordMain.id_relaboral);
    $("#hdnSwPrimeraVistaHistorialTurnAndExcept").val(0);
    definirGrillaParaListaMisEncuestasPorPersona(dataRecordMain);
}
/**
