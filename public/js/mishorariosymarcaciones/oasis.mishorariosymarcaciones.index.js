...
$().ready(function () {
    ...
    var dataRecord = getOneRelaboralById($("#hdnIdRelaboralUsuario").val());
    cargarHorariosPorPersona(dataRecord);
    $("#liHorarios").on("click",function () {
        $('#divTabControlMarcaciones').jqxTabs({selectedItem: 0});
        cargarHorariosPorPersona(dataRecord);
    });
    $("#liMarcaciones").on("click",function () {
        $('#divTabControlMarcaciones').jqxTabs({selectedItem: 1});
        cargarCalculosDeHorariosYMarcaciones(dataRecord);
    });
    ...
    $("#btnExportarCalculosExcel").click(function () {
        ...
        var fechaIni=$("#txtFechaCalculo").val();
        var fechaFin = $("#txtFechaCalculo").val();
        if (fechaIni!=''&&fechaIni!=undefined&&fechaFin!=''&&fechaFin!=undefined&&numColumnas > 0)
            if(numColumnas>1){
                exportarReporteCalculosHorariosYMarcacionesPersona(1,$("#hdnCiUsuario").val(),fechaIni,fechaFin);
            }else{
                var msje = "Debe seleccionar al menos dos columnas para la obtenci&oacute;n del c&aacute;lculo solicitado";
                $("#divMsjePorWarning").html("");
                $("#divMsjePorWarning").append(msje);
                $("#divMsjeNotificacionWarning").jqxNotification("open");
            }
        else {
            var msje = "Debe seleccionar al menos dos columnas para la obtención del reporte solicitado.";
            $("#divMsjePorWarning").html("");
            $("#divMsjePorWarning").append(msje);
            $("#divMsjeNotificacionWarning").jqxNotification("open");
        }
    ...
    $("#btnExportarCalculosPDF").click(function () {
        ...
        if (fechaIni!=''&&fechaIni!=undefined&&fechaFin!=''&&fechaFin!=undefined&&numColumnas > 0)
            if(numColumnas>1){
                exportarReporteCalculosHorariosYMarcacionesPersona(2,$("#hdnCiUsuario").val(),fechaIni,fechaFin);
            }else{
                var msje = "Debe seleccionar al menos dos columnas para la obtenci&oacute;n del c&aacute;lculo solicitado";
                $("#divMsjePorWarning").html("");
                $("#divMsjePorWarning").append(msje);
    ...
    $("#btnCalcular").on("click",function(){
        ...
        var fechaIni = $("#txtFechaCalculo").val();
        var fechaFin = $("#txtFechaCalculo").val();
        var ci = $("#hdnCiUsuario").val()+"";
        if (fechaIni!=''&&fechaIni!=undefined&&fechaFin!=''&&fechaFin!=undefined){
            if(numColumnas>1){
                ...
                definirGrillaMarcacionesYCalculosPorPersona(objParametro);
            }else{
                var msje = "Debe seleccionar al menos dos columnas para la obtenci&oacute;n del c&aacute;lculo solicitado";
            ...
            var msje = "Debe seleccionar la fecha para el cual se obtendr&aacute; el c&aacute;lculo.";
            $("#divMsjePorError").html("");
            $("#divMsjePorError").append(msje);
            $("#divMsjeNotificacionError").jqxNotification("open");
    ...
    $('#btnDesfiltrartodocalculos').click(function () {
        $("#divGridControlCalculos").jqxGrid('clearfilters');
    });
    $('#btnDesagrupartodocalculos').click(function () {
        $('#divGridControlCalculos').jqxGrid('cleargroups');
    });
        ...
        $("#divGridRelaborales").jqxGrid(
            {
                ...
                rendertoolbar: function (toolbar) {
                    ...
                    $("#calculaterowbutton").on('click', function () {
                        ...
                        $("#txtFechaCalculo").datepicker("update","");
                        $("#txtFechaCalculo").val('').datepicker('update');
                        $("#txtFechaCalculo").datepicker("update","");
                        $("#txtFechaCalculo").val('').datepicker('update');
                        $("#txtCiCalculo").val("");
                        var ci = '';
                    ...
                    $("#turnrowbutton").on('click', function () {
                        ...
                        if (selectedrowindex >= 0) {
                            ...
                            if (dataRecord != undefined) {
                                ...
                                var arrFechasPorSemana = iniciarCalendarioLaboralPorRelaboralTurnosYExcepcionesParaVerAsignacionesPorPersona(dataRecord,dataRecord.id_relaboral,5,idPerfilLaboral,tipoHorario,arrHorariosRegistrados,defaultGestion,defaultMes,defaultDia);
 ...
 * @returns {Array}
 */
function iniciarCalendarioLaboralPorRelaboralTurnosYExcepcionesParaVerAsignacionesPorPersona(dataRecord,idRelaboral,accion,idPerfilLaboral,tipoHorario,arrHorariosRegistrados,defaultGestion,defaultMes,defaultDia) {
...
/**
 * Función para cargar los horarios registrados para una persona por mes.
 * @param dataRecord
 */
function cargarHorariosPorPersona(dataRecord){
        $('#divTabControlMarcaciones').jqxTabs({selectedItem: 0});
    ...
    $("#ddEstadoDescripcionTurnAndExcept").html(dataRecord.estado_descripcion+"&nbsp;");
    $("#ddObservacionTurnAndExcept").html(dataRecord.observacion+"&nbsp;");
    if(dataRecord.estado==0){
        $("#dtFechaBajaTurnAndExcept").show();
        $("#ddFechaBajaTurnAndExcept").show();
 ...
 * @param dataRecord
 */
function cargarCalculosDeHorariosYMarcaciones(dataRecord){
    var date = new Date();
    var fecha = fechaConvertirAFormato(date,"-");
    $("#txtFechaCalculo").datepicker("update",fecha);
    var objParametro = {ci:dataRecord.ci,idOrganigrama : 0,idArea:0,idUbicacion:0,idMaquina:0,idRelaboral:dataRecord.id_relaboral,fechaIni:fecha,fechaFin:''}
    definirGrillaMarcacionesYCalculosPorPersona(objParametro);
}
/**
 * Función para cargar la grilla correspondiente al historial de Horarios y Marcaciones.
 * @param id_relaboral
 * @param id_persona
 * @param nombres
 * @param ci
 */
function cargarHistorialDeHorariosYMarcaciones(id_relaboral,id_persona,nombres,ci){
    ...
    definirGrillaParaListaControlMarcacionesDebidasYRealizadasPorIdRelaboralGestionMes(id_relaboral,id_persona,nombres,ci);
}
/**
