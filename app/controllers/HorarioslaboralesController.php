...
class HorarioslaboralesController extends ControllerBase
{
    ...
    public function indexAction()
    {
        ...
        $this->assets->addJs('/js/horarioslaborales/oasis.horarioslaborales.approve.js');
        $this->assets->addJs('/js/horarioslaborales/oasis.horarioslaborales.edit.js');
        $this->assets->addJs('/js/horarioslaborales/oasis.horarioslaborales.down.js');
    }

    /**
     * Función para la carga del primer listado sobre la página de gestión de horarios laborales.
    ...
    {   $msj = Array();
        ...
        if (isset($_POST["id"]) && $_POST["id"] > 0) {
            ...
            if($nombre!=''&& $color!=''&&$hora_entrada!=''&&$hora_salida!=''&&$hora_inicio_rango_ent!=''&&$hora_final_rango_ent!=''&&$hora_inicio_rango_sal!=''&&$hora_final_rango_sal!=''){
                $objHorarioLaboral = Horarioslaborales::findFirst(array("id=".$_POST["id"]));
                if(count($objHorarioLaboral)>0){
                    $cantMismoNombre = Horarioslaborales::count(array("id!=".$_POST["id"]." AND UPPER(nombre) LIKE UPPER('".$nombre."')"));
                    if ($cantMismoNombre == 0) {
                        $objHorarioLaboral->nombre = $nombre;
                        $objHorarioLaboral->nombre_alternativo = $nombre_alternativo;
                        $objHorarioLaboral->hora_entrada=$hora_entrada;
                        $objHorarioLaboral->hora_salida=$hora_salida;
                        $objHorarioLaboral->color=$color;
                        $objHorarioLaboral->fecha_ini=$fecha_ini;
                        $objHorarioLaboral->fecha_fin=$fecha_fin;
                        ...
                        $objHorarioLaboral->user_mod_id=$user_mod_id;
                        $objHorarioLaboral->fecha_mod=$hoy;
                        try{
                    ...
                    }else $msj = array('result' => 0, 'msj' => 'Error: Ya existe un horario con el mismo nombre.');
                }
            }else{
            ...
            $observacion = $_POST['observacion'];
            if($nombre!=''&& $color!=''&&$hora_entrada!=''&&$hora_salida!=''&&$hora_inicio_rango_ent!=''&&$hora_final_rango_ent!=''&&$hora_inicio_rango_sal!=''&&$hora_final_rango_sal!=''){
                $cantMismoNombre = Horarioslaborales::count(array("UPPER(nombre) LIKE UPPER('".$nombre."')"));
                if ($cantMismoNombre == 0) {
                    ...
                    $msj = array('result' => 0, 'msj' => 'Error: Ya existe un horario con el mismo nombre.');
                }
            }else{
                $msj = array('result' => 0, 'msj' => 'Error: Los datos enviados no cumplen los criterios necesarios para su registro.');
            }
    ...
    public function approveAction()
    {
        ...
        if (isset($_POST["id"]) && $_POST["id"] > 0) {
             ...
             */
            $objHorarioLaboral = Horarioslaborales::findFirstById($_POST["id"]);
            if ($objHorarioLaboral->id > 0 && $objHorarioLaboral->estado == 2) {
                try {
                    $objHorarioLaboral->estado = 1;
                    $objHorarioLaboral->user_mod_id = $user_mod_id;
                    $objHorarioLaboral->fecha_mod = $hoy;
                    $ok = $objHorarioLaboral->save();
                    if ($ok) {
    ...
    /**
     * Función para el la baja del registro de un horario laboral.
     * return array(EstadoResultado,Mensaje)
     * Los valores posibles para la variable EstadoResultado son:
     *  0: Error
     *   1: Procesado
    ...
    public function downAction()
    {
        ...
        try {
            if (isset($_POST["id"]) && $_POST["id"] > 0) {
                 ...
                 */
                $objHorarioLaboral = Horarioslaborales::findFirstById($_POST["id"]);
                $objHorarioLaboral->estado = 0;
                $objHorarioLaboral->user_mod_id = $user_mod_id;
                $objHorarioLaboral->fecha_mod = $hoy;
                if ($objHorarioLaboral->save()) {
                    $msj = array('result' => 1, 'msj' => '&Eacute;xito: Registro de Baja realizado de modo satisfactorio.');
                } else {
                foreach ($objHorarioLaboral->getMessages() as $message) {
                echo $message, "\n";
                }
                $msj = array('result' => -1, 'msj' => 'Error cr&iacute;tico: No se pudo dar de baja al registro de horario laboral.');
                }
            } else $msj = array('result' => -1, 'msj' => 'Error cr&iacute;tico: No se efectu&oacute; la baja debido a que no se especific&oacute; el registro de horario laboral.');
        } catch (\Exception $e) {
            echo get_class($e), ": ", $e->getMessage(), "\n";
            echo " File=", $e->getFile(), "\n";
            echo " Line=", $e->getLine(), "\n";
