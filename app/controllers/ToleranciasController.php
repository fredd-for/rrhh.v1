...
class ToleranciasController extends ControllerBase
{
    ...
    public function indexAction()
    {
        ...
        $this->assets->addCss('/assets/css/oasis.principal.css');
        $this->assets->addJs('/js/tolerancias/oasis.tolerancias.tab.js');
        $this->assets->addJs('/js/tolerancias/oasis.tolerancias.index.js');
        $this->assets->addJs('/js/tolerancias/oasis.tolerancias.new.edit.js');
        $this->assets->addJs('/js/tolerancias/oasis.tolerancias.approve.js');
        $this->assets->addJs('/js/tolerancias/oasis.tolerancias.export.js');
        $this->assets->addJs('/js/tolerancias/oasis.tolerancias.down.js');
    }
        ...
        if (isset($_POST["id"]) && $_POST["id"] > 0) {
            ...
            if($tolerancia>=0&&$tipo_acumulacion>=0&&$consideracion_retraso>=0){
                ...
                if(count($objTolerancia)>0){
                    $cantMismosDatos = Tolerancias::count(array("id!=".$_POST["id"]." AND tolerancia = ".$tolerancia." AND tipo_acumulacion=".$tipo_acumulacion." AND consideracion_retraso=".$consideracion_retraso.""));
    ...
    public function downAction()
    {
        ...
        try {
            if (isset($_POST["id"]) && $_POST["id"] > 0) {
                 ...
                 * Baja de registro
                 */
                $objTolerancia = Tolerancia::findFirstById($_POST["id"]);
